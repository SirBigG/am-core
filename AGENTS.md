...
- `docker compose exec core make test`: 225 core tests, 31 API tests, and flake8 passing.
...
- Step 6 security header coverage has started and now covers current `SecurityMiddleware`, clickjacking headers, a report-only CSP header, and representative public page groups. Admin assets, authenticated profile/diary CSP checks, and violation cleanup still remain.
