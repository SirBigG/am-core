...
- `GET`, `PUT`, or `PATCH /api/content/posts/<id>/` requires a token-authenticated `is_staff` user and can retrieve or update any post. The original publisher is preserved and cannot be changed through the request body. A non-staff token receives `403`.
